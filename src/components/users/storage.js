...
import {requestUserStorge,removeUserStorage,displayDateTime} from "../../services/ApiService"
...
function QuanLyData(props) {
  ...
  function refresh(){
    trackPromise(requestUserStorge(props.match.params.id,{...filterOption} ).then((data)=>{
  ...
  function setFilterOption(obj){
    ...
    trackPromise(requestUserStorge(props.match.params.id,obj).then((data)=>{
          setDatasource(data.content);
          var totalPage = data["totalPages"];
          var pages = [1];
  ...
  function remove(data){
    ...
    if (window.confirm ("Bọn có thực sự muốn xóa nội dung này ")){
        trackPromise(removeUserStorage(data.uuid,props.match.params.id).then(()=>{
          refresh()
        }),"loading")
    }
  }
function renderItem(m,index){
  ...
  return (<Content title="Danh sách bài hát của người dùng" browserTitle="Danh sách bài hát của người dùng">
      ...
      <Row>
       ...
       <Col xs={4}> <div style={{float:"right"}}>
        <Button icon="fa-arrow-left" type="default" text="Quay lại" onClick={()=>{ props.history.goBack() }} />
        </div></Col>

...
import {requestDataFilter,requestChangeAccount,displayDateTime} from "../../services/ApiService"
...
function QuanLyData(props) {
  ...
  useEffect(()=>{
    ...
    trackPromise(requestDataFilter({...filterOption} ).then((data)=>{
  ...
  function setFilterOption(obj){
    ...
    trackPromise(requestDataFilter(obj).then((data)=>{
          setDatasource(data.content);
          var totalPage = data["totalPages"];
          var pages = [1];
...
function renderItem(m,index){
  return <tr key={"item_"+(index+1)}>
    ...
    <Button icon="fa-edit"  text="Sửa" onClick = {()=>{
       props.history.push(baseurl+"/database/edit/"+m.uuid) 
    }}/>
    </td>
    </tr>
}
  return (<Content title="Quản lý Nhạc" browserTitle="Quản lý  Nhạc">
      ...
      <Row>
       ...
       <Col xs={4}> <div style={{float:"right"}}>
        <Button icon="fa-plus" type="primary" text="Thêm mới" onClick={()=>{ props.history.push(baseurl+"/database/add") }} />
        </div></Col>
